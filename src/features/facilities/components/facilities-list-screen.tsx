...
import type { ActiveState } from "../types/facility.types";
  ...
  return (
    <PageContainer>
      ...
      >
        ...
        {showCreate ? (
          <SectionCard title="Create facility" description="Codes are generated by backend services when left blank.">
            <FacilityForm
        ...
        {!facilitiesQuery.isLoading && !facilitiesQuery.error ? (
          <>
            <FacilitiesTable facilities={facilitiesQuery.data ?? []} canDeactivate={workspace.canDeactivateFacilities} onDeactivate={async (facility) => {
              if (!window.confirm(`Deactivate ${facility.name}?`)) return;
              await deactivateFacility.mutateAsync(facility.id);
            }} />
            <div className="space-y-4 md:hidden">
              {(facilitiesQuery.data ?? []).map((facility) => <FacilityMobileCard key={facility.id} facility={facility} canDeactivate={workspace.canDeactivateFacilities} onDeactivate={(target) => void deactivateFacility.mutateAsync(target.id)} />)}
