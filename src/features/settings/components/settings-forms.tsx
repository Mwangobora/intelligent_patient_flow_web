...
export function OrganizationForm({ initialValues, isSubmitting, onSubmit }: { initialValues?: Partial<OrganizationFormValues>; isSubmitting: boolean; onSubmit: Submit<OrganizationFormValues> }) {
  const form = useForm<OrganizationFormValues>({ resolver: zodResolver(organizationSchema), defaultValues: { name: initialValues?.name ?? "", legal_name: initialValues?.legal_name ?? "", email: initialValues?.email ?? "", phone_number: initialValues?.phone_number ?? "+255", registration_number: initialValues?.registration_number ?? "" } });
  return <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4"><FormErrorAlert message={form.formState.errors.root?.message} /><div className="grid gap-4 lg:grid-cols-2"><TextInputField label="Name" required error={form.formState.errors.name?.message} {...form.register("name")} /><TextInputField label="Legal name" {...form.register("legal_name")} /><TextInputField label="Email" type="email" {...form.register("email")} /><TextInputField label="Phone number" {...form.register("phone_number")} /><TextInputField label="Registration number" {...form.register("registration_number")} /></div><SubmitButton label="Save organization" loadingLabel="Saving..." isLoading={isSubmitting} /></form>;
}
...
export function UserForm({ initialValues, isSubmitting, onSubmit }: { initialValues?: Partial<UserFormValues>; isSubmitting: boolean; onSubmit: Submit<UserFormValues> }) {
  const form = useForm<UserFormValues>({ resolver: zodResolver(userSchema), defaultValues: { email: initialValues?.email ?? "", phone_number: initialValues?.phone_number ?? "+255", password: initialValues?.password ?? "", first_name: initialValues?.first_name ?? "", middle_name: initialValues?.middle_name ?? "", last_name: initialValues?.last_name ?? "" } });
